...
HOW TO:
First, create a txt file name "Streamers" exaclty without Quotes (has to be txt file)
In the "Streamers" Txt File input the Streamer name exactly from twitch's Url ex (https://www.twitch.tv/roflgator); roflgator, followed by newLine for each new streamer (Remove any "Blank" newLines after a new streamer)
...
A file called "LogFile" will be created, and updated, with detail log for each process while runing the script.
