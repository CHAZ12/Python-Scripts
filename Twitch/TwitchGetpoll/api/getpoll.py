...
def getpoll():
    ...
    if request.path == '/api/getytpoll':
        ...
        if title is None or choices is None:
            return 'Poll: Falied:(YT) no poll was found'
        elif len(title) > 60:
    ...
    #Get Poll values from Twitch
        ...
        if len(decoded_string) >= 2:
            values = decoded_string.replace(', ',',').split(',')
        if(len(values) < 4):
    ...
    if ytpoll == False:
        ...
        for i in range(len(values)):
            if(len(str(values[i])) > 25 and i != 0):
                print(f"Option, {i+1} was too long. Found: {len(str(values[i]))} chars in... '{values[i]}'")
                return f"Poll: Option was too long. Found: {len(str(values[i]))} chars in... '{values[i]},'"
            if(i == 0): # first entry is title
                title = values[i]
                if len(title) > 60:
                    print('Title was too long')
                    return(f'Poll: Title was too long, max is 60 char. FOUND:{len(title)}')
            elif(i == len(values)-1): # get last entry as time
                ready = True
